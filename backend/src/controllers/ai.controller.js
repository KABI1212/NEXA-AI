...
const asDataBlock = (label, value) => `${label}\n---BEGIN USER DATA---\n${JSON.stringify(value ?? "")}\n---END USER DATA---`;
...
export const careerExplorer = asyncHandler(async (req, res) => {
  ...
  const data = await generateAiJson(aiSystem, asDataBlock("Suggest careers for this profile data.", req.body), fallback);
...
export const resumeAnalyzer = asyncHandler(async (req, res) => {
  ...
  const data = await generateAiJson(aiSystem, asDataBlock("Analyze this resume text for ATS.", req.body.resumeText || "No text provided"), fallback);
...
export const interviewPrep = asyncHandler(async (req, res) => {
  ...
  const data = await generateAiJson(aiSystem, asDataBlock("Create interview prep for this company.", company), fallback);
...
export const companyInsights = asyncHandler(async (req, res) => {
  ...
  const data = await generateAiJson(aiSystem, asDataBlock("Give company insights for this company.", company), fallback);
...
export const skillGap = asyncHandler(async (req, res) => {
  ...
  const data = await generateAiJson(aiSystem, asDataBlock("Find skill gaps from this profile data.", req.body), fallback);
...
export const mentorChat = asyncHandler(async (req, res) => {
  ...
  const data = await generateAiJson(aiSystem, asDataBlock("Answer this mentoring question.", req.body.message), fallback);
  res.json({ data });
});
