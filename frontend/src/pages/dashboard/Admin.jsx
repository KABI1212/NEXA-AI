...
export default function Admin() {
  ...
  return <div className="grid gap-5">
    ...
    {overview && <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{Object.entries(overview).map(([k, v]) => <Card key={k} className="bg-white/95"><p className="text-sm font-bold uppercase text-slate-500">{k}</p><b className="text-3xl text-primary">{v}</b></Card>)}</div>}
    <Card className="bg-white/95">
      <h3 className="text-xl font-black">Upload Course Content</h3>
    ...
    <Card className="bg-white/95"><h3 className="text-xl font-black">Manage Users</h3><div className="mt-3 grid gap-2">{users.slice(0, 8).map((u) => <div key={u._id} className="flex justify-between rounded-md bg-slate-50 p-3"><span>{u.name} | {u.email}</span><b>{u.role}</b></div>)}</div></Card>
    <Card className="bg-white/95"><h3 className="text-xl font-black">View / Revoke Certificates</h3><div className="mt-3 grid gap-2">{certificates.slice(0, 8).map((c) => <div key={c._id} className="flex items-center justify-between rounded-md bg-slate-50 p-3"><span>{c.certificateId} | {c.courseName}</span><Button variant="outline" onClick={() => revoke(c._id)} disabled={!c.verified}>{c.verified ? "Revoke" : "Revoked"}</Button></div>)}</div></Card>
  </div>;
}
