    ...
    <div className="grid lg:grid-cols-[280px_1fr]">
      <aside className="sticky top-0 h-auto border-b border-white/10 bg-black/10 p-4 lg:h-screen lg:border-b-0 lg:border-r">
        ...
        <nav className="grid max-h-[72vh] gap-1 overflow-auto pr-1">
          {links.map(([label, path, Icon]) => <NavLink key={path} to={path} end={path === "/dashboard"} className={({ isActive }) => `flex items-center gap-3 rounded-md px-3 py-2 text-sm font-semibold ${isActive ? "bg-white text-navy" : "text-blue-100 hover:bg-white/10"}`}><Icon size={18} />{label}</NavLink>)}
          {user?.role === "admin" && <NavLink to="/dashboard/admin" className={({ isActive }) => `flex items-center gap-3 rounded-md px-3 py-2 text-sm font-semibold ${isActive ? "bg-white text-navy" : "text-blue-100 hover:bg-white/10"}`}><Shield size={18} />Admin Panel</NavLink>}
        </nav>
      ...
      <main className="min-w-0 p-4 lg:p-7">
        <div className="mb-6 flex items-center justify-between">
          <div><p className="text-sm text-blue-100">Welcome back</p><h1 className="text-2xl font-black">{user?.name || "Nexa Learner"}</h1></div>
